...
def webCreate():
    app.layout = html.Div(children=[
        ...
        '''),
        html.Div(children='Katulski Adrian'),
        html.Div(children=vinNumber),
            ...
            figure=dict(
                data=[
                    dict(
                        ...
                        name='% - ilość paliwa',
                        marker=dict(
                            color='rgb(255, 00, 102)'
                        )
                    ),
                    dict(
                        y=enginerpm,
                        name='obr./min - prędkość obrotowa wału ',
                        marker=dict(
                            color='rgb(255, 124, 00)'
                        )
                    ),
                    dict(
                        y=vehiclespeed,
                        name='km/h - prędkość pojazdu',
                        marker=dict(
                            color='rgb(00, 255, 00)'
                        )
                    ),
                    dict(
                        y=enginecoolant,
                        name='°C - temperatura płynu chłodnicznego',
                        marker=dict(
                            color='rgb(255, 00, 00)'
                        )
                    ),
                    dict(
                        y=batteryvoltage,
                        name='V - napięcie na akumulatorze',
                        marker=dict(
                            color='rgb(00, 00, 255)'
                        )
                    )
...
async def sendRequest(bus, pid, serwis, repeat, sleep):
...
async def main():
    ...
    await sendRequest(bus=can0, pid=[0x0d, 0x0c, 0x05, 0x42, 0x2f], serwis=0x01, repeat=40, sleep=0.2)
    GP.output([6, 5], [GP.LOW, GP.HIGH])
    print(str(fueltank) + ' ilosc paliwa % ' + str(len(fueltank)))
    print(str(enginerpm) + ' obroty silnika rpm ' + str(len(enginerpm)))
    print(str(vehiclespeed) + ' prędkość samochodu km/h ' + str(len(vehiclespeed)))
    print(str(enginecoolant) + ' temperatura płynu chłodzącego ' + str(len(enginecoolant)))
    print(str(batteryvoltage) + ' napięcie akumulatora ' + str(len(batteryvoltage)))
    webCreate()
